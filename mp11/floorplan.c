...
// Intro Paragraph: In this mp, I created trees and some submethods that help solving floor plan problems
...
int is_leaf_node(node_t* ptr) {
  ...
  //return 1 if left and right child doesn't exist
...
int is_internal_node(node_t* ptr) {
  // TODO: (remember to modify the return value appropriately)
  // Return 1 if the ptr is not a leaf node
...
int is_in_subtree(node_t* a, node_t* b) {
  // TODO: (remember to modify the return value appropriately)
  // Base cases: if a or b is null, return 0. If a is equal to b, return 1
  // Otherwise solve the problem recursively with left and right child 
  if(a==NULL||b==NULL)
  {
    return 0;
  }
...
void recut(node_t* ptr) {
  ...
  // TODO: if cutline is H, change it to V and vice versa
  if(ptr->cutline==H)
  {
    ptr->cutline=V;
  }
...
void swap_module(node_t* a, node_t* b) {
  ...
  // TODO:
  module_t *swap=(module_t *)malloc(sizeof(module_t));
  swap=a->module;
  a->module=b->module;
  b->module=swap;
  return;
...
void swap_topology(node_t* a, node_t* b) {
  ...
  // TODO: if a is the right child, set right child to b
  // if a is the left child, set left child to b
  // if b is the right child, set right child to a
  // if b is the left child, set left child to a
  node_t *a_temp=a;
  ...
  if(b_parent->right==b)
  {
    b_parent->right=a_temp;
    a_temp->parent=b_parent;
  }
  else if(b_parent->left==b)
  {
    b_parent->left=a_temp;
    a_temp->parent=b_parent;
  }
}
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  ...
  // TODO: resursively calling left and right child
  // set module to pointers module if cutline is undefined
  // set cutling to pointers cutline otherwise
...
{
  // TODO: if a node is a leaf node, add up its resource and recursively call its left and right child.
  int ans=0;
  if(ptr!=NULL)
  {
    if(is_leaf_node(ptr))
    {
      ans+=ptr->module->resource;
    }
    ans+=get_total_resource(ptr->left);
    ans+=get_total_resource(ptr->right);
  }
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  if(n==x)
  {
    ...
    ptr->parent=par;
    ptr->cutline=UNDEFINED_CUTLINE; //initiate
    return ptr;
  }
  else{
    ptr->module=NULL; //otherwise set module to null
